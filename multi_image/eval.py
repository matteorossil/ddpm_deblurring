...
class Evaluator():
    ...
    def __init__(self, argv):
        ...
        self.denoiser = Denoiser(
            image_channels=self.image_channels*2+6,
    ...
    def sample_(self, mode, path, psnr_init, ssim_init, psnr_deblur, ssim_deblur):
        ...
        with torch.no_grad():
            ...
            for idx, ((sharp_left, blur_left), (sharp, blur), (sharp_right, blur_right)) in enumerate(dataloader):

                # Move data to device
                sharp_left = sharp_left.to(self.device)
                blur_left = blur_left.to(self.device)

                sharp = sharp.to(self.device)
                blur = blur.to(self.device)

                sharp_right = sharp_right.to(self.device)
                blur_right = blur_right.to(self.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default="gopro2")
